...
//NewRaw creates a Fixed for an integer
func NewR(raw int64) Fixed {
	return Fixed{fp: raw}
}
...
// Reverse returns the value after the reversal
func (f Fixed) Rev() Fixed {
	if f.IsNaN() {
		return NaN
	}
	f0 := Fixed{fp: f.fp * -1}
...
func (f Fixed) Mul(f0 Fixed) Fixed {
	...
	if fp0a != 0 {
		result = fpa*fp0a*scale + fpb*fp0a
	}
	if fp0b != 0 {
		result = result + (fpa * fp0b) + ((fpb)*fp0b)/scale
	}
